...
public class BoardsRuntimeConfiguration {
  ...
  BoardsRuntimeGuard boardsRuntimeGuard(
      @Value("${weave.boards.workspace.runtime-enabled:${weave.boards.runtime-enabled:false}}")
          boolean enabled) {
  ...
  BoardsRepository boardsRepository(
      @Value("${weave.boards.workspace.provider:${weave.boards.provider:local-workspace}}")
          String provider,
      @Value("${weave.boards.openproject.provider-runtime-enabled:false}")
          boolean openProjectRuntimeEnabled,
      @Value("${weave.boards.openproject.read-sync-enabled:false}")
          boolean openProjectReadSyncEnabled,
      @Value("${weave.boards.openproject.context-authorization-enabled:false}")
          boolean openProjectContextAuthorizationEnabled,
      @Value("${weave.boards.openproject.audit-consent-enabled:false}")
          boolean openProjectAuditConsentEnabled,
      @Value("${weave.boards.openproject.provider-writes-enabled:false}")
  ...
  @ConditionalOnProperty(
      name = "weave.audit.events.storage.mode",
      havingValue = "file",
      matchIfMissing = true)
  ...
  // Retained for direct unit tests that instantiate this configuration without a Spring

...
public final class RuntimeProfileIssuanceService {
    ...
    public SignedRuntimeProfile issue(RuntimeCell expectedCell, String issuanceRef, Instant issuedAt) {
        ...
        if (expectedCell.entitlementState() != RuntimeEntitlementState.ENTITLED) {
            throw new IllegalStateException("A RuntimeProfile cannot be issued for an inactive entitlement");
        }
    ...
    private static String profileId(RuntimeCell cell, String issuanceRef) {
        String semantics = "weave.runtime-profile-id/v2\u0000"
        ...
        try {
            byte[] digest = MessageDigest.getInstance("SHA-256")
                    .digest(semantics.getBytes(StandardCharsets.UTF_8));

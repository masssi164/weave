 ...
 *
 * <p>The projection deliberately contains no private-key bytes or filesystem
 * locations. Implementations must make every operation idempotent by its
 * operation reference and must keep the previous public key trusted for the
 * complete configured overlap window.</p>
 */
public interface RuntimeProfileSigningKeyLifecycle {
    ...
    record KeyRingState(
        ...
        public KeyRingState {
            if (activeKeyId == null || activeKeyId.isBlank() || keys == null || keys.isEmpty()) {
                throw new IllegalArgumentException("a support-safe active signing-key projection is required");
            }
    ...
    record PublishedKeyState(
        ...
        public PublishedKeyState {
            if (keyId == null || keyId.isBlank() || status == null || validFrom == null || validUntil == null

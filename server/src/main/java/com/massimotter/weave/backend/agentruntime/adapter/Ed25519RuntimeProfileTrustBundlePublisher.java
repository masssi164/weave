...
public final class Ed25519RuntimeProfileTrustBundlePublisher implements RuntimeProfileTrustBundlePublisher {
    ...
    public Optional<RuntimeProfileJwkSet> publish(Instant now) {
        ...
        List<RuntimeProfileTrustKeyProvider.TrustKey> current = trustKeys.publishedKeys(now).stream()
        ...
        for (RuntimeProfileTrustKeyProvider.TrustKey key : current) {
            if (!keyIds.add(key.keyId())) {
                throw new IllegalStateException("RuntimeProfile trust provider returned an ambiguous key id");
            }
            ...
            projected.add(new RuntimeProfileJwkSet.Jwk(
                    "OKP", "Ed25519", encodePoint(ed25519), "sig", "EdDSA", key.keyId()));
        }
        return projected.isEmpty() ? Optional.empty() : Optional.of(new RuntimeProfileJwkSet(projected));
    }

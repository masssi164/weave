...
- OIDC-gated Matrix Client-Server facade at `/_matrix/client/**`: member room sync, timeline, send, receipt, typing, reaction, redaction, room lifecycle, and identity operations project the canonical Chat domain through the shared Rust/Ruma JNI core. Spring authorization and `chat.read`/`chat.send` capability checks run before the replaceable southbound `ChatProviderPort`; writes are audited without exposing provider payloads. This is a Weave protocol facade, not a northbound Synapse or other homeserver.
- Chat control and context APIs at `/api/chat/readiness`, `/api/v1/chat/**`, and `/api/admin/chat/**`: member routes retain support-safe readiness, decisions, meeting capsules, and read-only Weaver scout context; admin routes retain selected Chat mapping and audited migration dry-run/preflight reports. Deprecated REST conversation/message data-plane routes are unavailable.

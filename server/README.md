...
- Canonical domain registry v1 in `/api/providers/status` and `src/main/resources/contracts/canonical-domain-registry.v1.json`, covering identity, people, spaces, chat, files, documents, calendar, boards, calls, decisions, notifications, health, and Weaver with member/admin states, compatibility aliases, portability metadata, and no-unaccounted-data-loss migration primitives.

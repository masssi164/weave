...
| Agent Runtime Control | Guarded: per-cell Keycloak identity, RFC 9068 workload admission, token exchange, RuntimeProfile v2/current ARC binding, deletion, and restore reconciliation are implemented; Weaver/OpenClaw is the first runtime provider and domain tool catalogs remain empty until current authorization, ApprovalDecisionEvidence v2, and ActionEvidence v2 are complete | `weave-mcp-server/src/test/java/com/massimotter/weave/mcp/SpringAiMcpTransportTest.java`, `server/src/test/java/com/massimotter/weave/backend/agentruntime/application/McpWorkloadAuthorizationServiceTest.java`, `docs/weave-contract-java-mcp.md` |

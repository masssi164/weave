...
| Calls/Meetings | iOS CallKit + PushKit/VoIP concerns; Android Core-Telecom where supported. | Matrix Client-Server v1.19 plus pinned MatrixRTC Profile 0 owns signaling and membership. A thin native coordinator maps OS lifecycle to MatrixRTC/media state; LiveKit remains transport-only. | Availability is Guarded until exact wire, RTC authorization, media E2EE, TURN/reconnect, physical-device, consent, artifact, and accessibility evidence passes. |

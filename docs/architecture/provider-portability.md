...
| --- | --- | --- | --- |
| Files | iOS File Provider extension; Android DocumentsProvider / Storage Access Framework. | `GET /api/files/native-provider-setup` plus file facade roots/list/open/upload hooks. The response contains Weave API paths only. | Full native availability is blocked until iOS/Android provider implementations list/open at least one Weave file through the facade and revocation is proven on device. |

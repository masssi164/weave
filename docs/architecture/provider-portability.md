...
| --- | --- | --- | --- |
| Files | iOS File Provider extension; Android DocumentsProvider / Storage Access Framework. | `GET /api/files/native-provider-setup` for setup/readiness plus `/dav/files` WebDAV facade roots/list/open hooks. The response contains Weave paths only; writes remain blocked by #1007 until ETag, conflict, lock, quota, revocation, and audit policy is evidenced. | Full native availability is blocked until iOS/Android provider implementations list/open at least one Weave file through the facade and revocation is proven on device. |

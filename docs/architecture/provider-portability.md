...
| Calls/Meetings | iOS CallKit + PushKit/VoIP concerns; Android Core-Telecom where supported. | Matrix v1.19 plus pinned MatrixRTC Profile 0 is the member signaling contract. An internal RTC Authorizer exchanges independently authorized Matrix room/slot/member/device context for a short-lived SFU token; no member Calls API exists. | Full native availability is blocked until Profile 0 interoperability, RTC authorization/revocation, MatrixRTC media E2EE, native call UI, permissions, audio routing, TURN/reconnect, and physical-device evidence pass. |

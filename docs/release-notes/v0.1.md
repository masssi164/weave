...
- Weaver/OpenClaw release documentation clarifies the governed runtime boundary: Weave projects a stable `channels.weave-chat` channel backed by Weave Chat-domain routing, model aliases, MCP/tool/skill grants, CredentialRefs and short-lived runtime token references, sandbox policy, and audit correlation into the OpenClaw-derived runtime, while Weave MCP remains the policy-enforcement point for governed tool calls and raw OpenClaw configuration stays out of member UX.

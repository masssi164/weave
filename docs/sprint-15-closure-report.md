...
- Effective policy preview explains roles, groups, context roles, provider mappings, grants, denies, readiness/member-impact states, unknown-input fail-closed behavior, Weaver runtime disabled-by-default posture, and audit refs before member go-live or a reviewed Identity Ops apply.
...
| --- | --- | --- |
| [weave#558](https://github.com/masssi164/weave/issues/558) | `AdminControlPlaneControllerTest` covers admin-only control plane, read-only identity readiness, existing/new organization bootstrap, effective-policy simulation, operator read/no-write boundary, member denial, SecretRef-only provider configuration, and redacted audit. The later native-Organizations refactor retired the former server realm-provider tests; `infra/weave-workspace/tests/identity_ops_contract_test.py`, the profile-specific Identity Ops tasks, `docs/organization-embedding-contract.md`, `docs/identity-provisioning-strategy.md`, and `docs/admin-operator-handbook.md` now cover the single-writer baseline, exact organization roles/capabilities, immutable identities, deny-by-default policy, and support-safe readiness. | Satisfied; implementation evidence superseded by Identity Ops. |

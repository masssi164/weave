...
Make the local dogfood stack DNS-first for `*.weave.local`, keep any LAN host/IP input non-canonical, bootstrap the local CA from `weave.local`, and prove that a member can join Weave by invitation link, QR code, or normal credential login without seeing raw provider configuration.

## Governing context

- Current branch: `feat/enterprise-app-start-ux`.
- Current dogfood contract: DNS-first `weave.local` / `*.weave.local`; LAN IPs are not canonical app, issuer, service, or CA URLs.
- New operator fact: Massimo added LAN DNS for `*.weave.local`.
...
- `infra/weave-workspace/01-infrastructure/main.tf`
  - Reconciled app, API, issuer, Matrix, files, and CA public outputs to DNS-first `weave.local` / `*.weave.local` values.
...
- `infra/weave-workspace/01-infrastructure/templates/Caddyfile.tpl`
  - Uses DNS-first default SNI on `weave.local`.
  - Adds HTTP CA download at `http://weave.local:44080/weave-local-ca.pem` and HTTPS CA download at `/weave-local-ca.pem`.
...
- `curl http://weave.local:44080/weave-local-ca.pem` returned `200`.
- `curl --cacert ... https://weave.local:44443/weave-local-ca.pem` returned `200`.
- `curl --cacert ... https://api.weave.local:44443/api/platform/config` returned `200` and DNS-first URLs.
- `openssl s_client -connect api.weave.local:44443 -servername api.weave.local -CAfile .../weave-local-ca.pem -verify_hostname api.weave.local` returned `Verify return code: 0 (ok)`.
...
- Public app/server config uses DNS-first URLs:
  - Product: `https://weave.local:44443` when isolated ports are active, or `https://weave.local` on canonical ports.
  - API: `https://api.weave.local:44443/api` or canonical-port equivalent.
  - Auth/IAM: `https://auth.weave.local:44443/realms/weave` or canonical-port equivalent.
  - Matrix: `https://matrix.weave.local:44443` or canonical-port equivalent.
  - Files technical fallback: `https://files.weave.local:44443` or canonical-port equivalent.
...
- HTTP CA bootstrap is advertised DNS-first only:
  - `http://weave.local:44080/weave-local-ca.pem`
...
### 1. Local infra setup — DNS-first `*.weave.local` profile

Acceptance criteria:

- `install.sh` supports a DNS-first local profile where generated public and CA URLs stay on `weave.local` / `*.weave.local`; stale LAN-IP inputs do not create a second truth.
- Generated Caddy config serves `weave.local`, `api.weave.local`, `auth.weave.local`, `matrix.weave.local`, `files.weave.local`, and `admin.weave.local` over the same central local CA.
- The generated certificate includes `DNS:weave.local` and `DNS:*.weave.local`; if compatibility needs concrete names, the concrete service hosts are included too.
- `openssl s_client -connect api.weave.local:44443 -servername api.weave.local -CAfile .../weave-local-ca.pem -verify_hostname api.weave.local` verifies successfully.
- `curl --cacert ... https://api.weave.local:44443/api/platform/config` returns `200` and DNS-first URLs.
- The DNS-first CA bootstrap endpoint `http://weave.local:44080/weave-local-ca.pem` returns the CA with `200`; IP CA advertisement is not canonical evidence.
- No service uses `--insecure`, disabled TLS validation, or certificate-ignore flags as final evidence.

Recommended issue draft:

```markdown
Title: infra(local): make dogfood stack DNS-first for *.weave.local with DNS CA bootstrap

Body:
Implement the Sprint 32 local DNS correction. The local stack must use `*.weave.local` public URLs now that LAN DNS exists. Root CA bootstrap is also DNS-first via `weave.local`; LAN IPs are non-canonical debug data only.

Acceptance:
- `install.sh` has a DNS-first mode/profile for `weave.local`, `api.weave.local`, `auth.weave.local`, `matrix.weave.local`, `files.weave.local`, and `admin.weave.local`.
- Generated Caddy certificate covers `DNS:weave.local`, `DNS:*.weave.local`, and concrete service hosts as needed.
- `/weave-local-ca.pem` is advertised over HTTP on `weave.local`.
- `curl --cacert ... https://api.weave.local:44443/api/platform/config` returns `200` and DNS-first service URLs.
- `openssl s_client -servername api.weave.local -verify_hostname api.weave.local` verifies successfully with the generated CA.
...
- Backend issuer validation expects `https://auth.weave.local[:port]/realms/weave`; JWKS can still be fetched over the internal Docker network.
...
Body:
Ensure the local backend/server setup consumes the DNS-first local contract instead of exposing a single LAN-IP origin when `*.weave.local` is available.

Acceptance:
- `/api/platform/config` returns DNS-first product, API, auth, Matrix, files, and calendar URLs.
- Keycloak issuer is `https://auth.weave.local[:port]/realms/weave` and backend token validation accepts that issuer.
...
Evidence:
- `curl --cacert ... https://api.weave.local:44443/api/platform/config`
- Keycloak discovery curl against `auth.weave.local`
- Matrix well-known/auth metadata curl against `matrix.weave.local`
- `./smoke-test.sh`
```
...
Evidence:
- `openssl x509 -in .../weave.local.pem -noout -text` SAN excerpt
- service trust setup diff
- `openssl s_client` hostname verification for `api.weave.local`
- CA download curl over `weave.local`
```
...
- The member app accepts `https://weave.local[:port]/join?...` and/or `https://join.<tenant-domain>/join?...` when that domain is LAN DNS-resolvable and covered by the local CA.
- The parser no longer treats all `.local` hostnames as forbidden when the host is the intentional `weave.local` local dogfood domain or a configured LAN DNS domain.
...
- Tests cover DNS-first `.weave.local`, rejected LAN-IP app-start links, production DNS, and forbidden loopback/container cases.

Recommended issue draft:

```markdown
Title: client(onboarding): accept DNS-first weave.local join links without weakening unsafe local-target guards

Body:
Update the member handoff parser and onboarding tests for Sprint 32. Massimo's LAN now resolves `*.weave.local`, so `.weave.local` must be valid for the local dogfood profile while loopback and container-only targets stay blocked.

Acceptance:
- `https://weave.local:44443/join?...` parses for local dogfood when backed by `/api/platform/config`.
- `https://api.weave.local:44443/api/platform/config` is accepted as explicit discovery URL.
...
- Member can alternatively enter the organization IAM/auth URL, such as `https://auth.weave.local[:port]/realms/weave`, and the app can discover or derive the platform config through an explicit supported contract rather than guessing provider topology.
...
- Generated app, service, issuer, Matrix, files, and CA URLs are DNS-first on `weave.local` / `*.weave.local`.
...
- Certificate SAN generation includes `weave.local`, `*.weave.local`, and concrete service hostnames.
- Caddy advertises CA bootstrap on `http://weave.local:44080/weave-local-ca.pem`.
- App handoff validation permits the intentional `weave.local` / `*.weave.local` LAN DNS domain and rejects LAN-IP app-start links, loopback, container-only names, and unrelated `.local` names.
...
- `.local` is often mDNS-reserved. Massimo's LAN DNS resolves `*.weave.local`, but the app and docs must distinguish intentional LAN DNS from unsafe Mac-only assumptions.
...
- A wildcard cert may not satisfy every service/tooling check if the checked hostname is the apex `weave.local`; include both apex and wildcard.
...
- `curl http://weave.local:44080/weave-local-ca.pem`
- `curl --cacert infra/weave-workspace/01-infrastructure/.generated/caddy/certs/weave-local-ca.pem https://api.weave.local:44443/api/platform/config`
- `openssl s_client -connect api.weave.local:44443 -servername api.weave.local -CAfile infra/weave-workspace/01-infrastructure/.generated/caddy/certs/weave-local-ca.pem -verify_hostname api.weave.local`
- `cd client && flutter test test/features/onboarding/member_handoff_test.dart test/features/onboarding/consume_member_handoff_test.dart`
...
dig +short weave.local
dig +short api.weave.local
dig +short auth.weave.local
...
openssl s_client \
  -connect api.weave.local:44443 \
  -servername api.weave.local \
  -CAfile 01-infrastructure/.generated/caddy/certs/weave-local-ca.pem \
  -verify_hostname api.weave.local

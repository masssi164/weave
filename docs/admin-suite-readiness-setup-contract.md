...
Sprint 19 go-live control room extension for #600: the RC claim-control summary must explicitly surface current accessibility, CI, Live Stack, release notes, support-bundle, audit/export, migration, Weaver, and release-owner evidence state. Historical issue #591 is closed and must not be surfaced as an active veto; missing or stale current evidence still blocks public/final release and unsupported accessibility/production-readiness wording, but must not block internal dogfood planning, implementation, or control-room evidence work.

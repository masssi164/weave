...
The dedicated runner removes stale Weave-generated outputs before checkout and requires at least 10 GiB of free space before tool and image setup, then checks for 4 GiB again immediately before native tests. Root CI owns generated-source freshness, so the live behavior lane does not repeat build-runner generation. Native tests hold a runner-owned 1 GiB emergency reserve; a background monitor releases it below a 2 GiB logging margin and fails the run support-safely, preserving enough space for diagnostics, targeted teardown, evidence upload, and final cleanup. The runner verifies the generated local certificate chain, then supplies that public CA to the Rust Matrix client as an explicit extra root for live tests only. Reading the runtime CA path also requires a compile-time live-test `dart-define` that defaults to false. The Rust client retains platform roots, certificate-chain validation, and hostname validation; production and physical-device builds receive no injected root and continue to use platform trust. Current Flutter/Rust native outputs remain untouched while tests, diagnostics, and evidence generation run; cleanup occurs only after acceptance evidence upload. The cleanup boundary is limited to generated paths inside the runner's Weave checkout, the named live-stack temporary evidence/reserve files, the named Weave stack containers and volumes, and the two locally built Weave image tags. It must not prune unrelated containers, volumes, signing identities, or physical-device data.

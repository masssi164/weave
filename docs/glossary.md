...
- **Weaver**: Separate OpenClaw-derived AI harness/runtime used by a member to communicate with Weave. A Weaver instance is a per-user OpenClaw-derived AI harness/runtime reached through the Weave channel and governed by Weave policy, user rights, organization allowlists, approvals, sandboxing, and audit.
- **AI harness**: The execution runtime shape used to run an assistant turn. For Weaver, the harness is OpenClaw-derived: it can run an agent, receive channel turns, use tools, apply policy, keep sessions, and emit audit/support evidence.
- **OpenClaw-derived Weaver runtime**: A Weaver runtime generated from a Weave `WeaverRuntimeProfile` and based on OpenClaw runtime concepts/capabilities. It is not a raw member-editable OpenClaw install. Weave generates the profile, channel projection, tool grants, MCP entries, sandbox policy, model aliases, and approval rules.

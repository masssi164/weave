...
| `mcp-domain-boundary` | MCP/Agents | Spring AI 2.0 stateful Streamable HTTP, protected-resource discovery, RFC 9068 workload admission, the MCP Client Credentials extension, token exchange, and current ARC/backend context resolution are implemented. Domain tool/resource/prompt catalogs remain empty until catalog/profile/current-authorization intersection and approval/action-evidence gates are complete. Calls MCP remains blocked on the Calls profile. |

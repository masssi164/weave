...
| Matrix Chat dry-run/consequence evidence | "Admins can review backend-owned Matrix Chat dry-run consequences before any future provider-switch apply." | Server/Admin Console tests + operator runbook evidence | `POST /api/admin/providers/replacements/dry-run`, Admin Console consequence preview, `docs/matrix-chat-sprint15-dry-run-policy.md`, and `./gradlew serverCi adminCi portabilityContractCheck`. | Usable as dry-run/consequence evidence only; do not claim production Matrix migration, automated switch, lossless migration, legal compliance, or E2EE history migration. |

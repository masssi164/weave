...
| Weave Control bootstrap-to-client boundary | "Weave Control separates deploy-new, attach-existing, and hybrid admin setup from the normal member app entry." | Contract docs + client architecture tests + future E2E evidence | `docs/weave-control-bootstrap-to-client-contract.md`, first-use/provider-boundary docs, and client architecture guards. | Contract-only until #665 local Forgejo deployed-stack evidence and #591 manual AT decision are complete; do not claim production mutation, full deploy/attach completion, or default Weaver PA availability. |
...
| Bounded governed Weaver assistance | "Weaver can provide bounded workspace assistance only through a signed same-user RuntimeProfile, policy/consent/scoped grants, approval receipts for shared-state changes, support-safe audit refs, and canonical Space/Decision/Board references." | Security/contract tests + claim matrix entry | `WeaverRuntimeServiceTest`, `WeaverToolRegistryTest`, governed Weaver runtime security contract, and Weaver support-safe evidence report. | Guarded: usable only as bounded governed-assistance evidence; do not claim broad autonomous AI availability, default enablement, unrestricted tools, background agents, raw provider access, or shared-state writes without approval receipts. |
...
| Release claim-control evidence | "Sprint 18 release trust is claim-controlled: CI, Live Stack, release notes, accessibility, support bundle, audit/export, migration, Weaver, and blocker evidence must be current before RC or production wording is allowed." | Claim matrix entry + Accessibility/manual UX evidence + Operator runbook evidence | `release/accessibility-gate.json`, `docs/evidence/accessibility/sprint-18-manual-at-blocker.md`, `docs/admin-suite-readiness-setup-contract.md`, release readiness checks, and issue #591. | Guarded: manual assistive-technology signoff remains blocked by #591; production cutover remains unavailable; support-bundle, audit, export/import, release notes, CI, and Live Stack references are support-safe evidence pointers, not proof of final release approval. |
| Sprint 32 Beta readiness slice | "Sprint 32 Beta readiness is an end-to-end Admin + User + governed Weaver + foundation product state, with adapter switching blocked unless dry-run evidence accounts for data loss." | Claim matrix entry + CI + E2E + migration dry-run + accessibility smoke + release-note evidence | `docs/beta-readiness-claim-gates.md`, GitHub issues #830-#836, #829 as foundation cleanup only, issue-specific CI, Admin/User/Weaver E2E, adapter migration dry-run evidence, accessibility smoke evidence, and release notes aligned by #836. | Guarded: do not claim Beta completion from #829, a single MCP smoke, or component CI. MCP is an internal governed bridge, not the user-facing product surface. Production migration apply, public launch/GA, lossless provider switching, unrestricted autonomous AI, and raw provider access remain blocked unless a later claim gate promotes them. |

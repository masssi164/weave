...
The single-host operator path needs a support-safe way for an operator to activate a Weave user without editing Keycloak internals by hand. The helper is intentionally local/dev oriented and maps directly to the current backend product-profile contract:

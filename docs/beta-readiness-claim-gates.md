...
- **Foundation perspective:** canonical domain cores, server standard facades, the workload-only MCP projection, adapters, migration/dry-run evidence, support bundles, and release evidence form the proof boundary. Adapter switching without unaccounted data loss is a first-class Beta requirement, not a future enhancement.
...
| Beta readiness is end-to-end across Admin, User, Weaver, and foundation components. | Admin Console/`weave-ctrl`, Weave Client, Weaver, canonical domain cores, server standard facades, MCP projection, adapters, evidence. | CI, E2E, migration dry-run, a11y smoke, release notes. | #831-#836 complete; `./gradlew docsCheck`; issue-specific CI; #835 E2E evidence; #836 release evidence; milestone closure report. | Blocked until the full issue set is closed with evidence. |
| The MCP projection is aligned to the canonical server domain facades. | server standard projections, `weave-mcp-server`, canonical domain dispatch. | CI and server tests from #829 plus the Files MCP projection contract. | #829 merged with server gate evidence. | Proven as foundation cleanup only; not a standalone Beta completion claim. |

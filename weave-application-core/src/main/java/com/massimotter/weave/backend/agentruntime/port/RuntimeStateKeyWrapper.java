...
public interface RuntimeStateKeyWrapper {
    ...
    record KeyReadiness(boolean ready, String activeKeyRefHash) {
        public KeyReadiness {
            if (ready && (activeKeyRefHash == null
                    || !activeKeyRefHash.matches("sha256:[a-f0-9]{64}"))) {
                throw new IllegalArgumentException("ready key custody requires a support-safe key reference hash");
            }
            if (!ready) {

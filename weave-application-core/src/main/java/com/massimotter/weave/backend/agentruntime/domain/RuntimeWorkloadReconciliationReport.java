  ...
  public RuntimeWorkloadReconciliationReport {
    ...
    if (!GUARDED_CLAIM.equals(capabilityClaim)) {
      throw new IllegalArgumentException(
          "Workload identity remains guarded until full live evidence passes");
    }
    if (state == null
        || observedAt == null
        || nextReconcileAt == null
        || !nextReconcileAt.isAfter(observedAt)) {
      throw new IllegalArgumentException(
          "Reconciliation state and observation window are required");
    }

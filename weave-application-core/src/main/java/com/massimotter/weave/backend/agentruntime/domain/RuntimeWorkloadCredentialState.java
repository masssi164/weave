...
public record RuntimeWorkloadCredentialState(
  ...
  public RuntimeWorkloadCredentialState {
    ...
    acceptedKeyIds =
        acceptedKeyIds == null ? Set.of() : Set.copyOf(new LinkedHashSet<>(acceptedKeyIds));
    if (acceptedKeyIds.isEmpty()
        || !acceptedKeyIds.contains(activeKeyId)
        || acceptedKeyIds.stream().anyMatch(value -> !KEY_ID.matcher(value).matches())) {
      throw new IllegalArgumentException(
          "acceptedKeyIds must contain only valid key ids and include the active key");
    }
    ...
    if (rotationPhase == RotationPhase.NONE) {
      if (rotationFingerprint != null || acceptedKeyIds.size() != 1) {
        throw new IllegalArgumentException(
            "non-rotating credentials must expose one accepted key and no rotation fingerprint");
      }
    } else {
      requireFingerprint(rotationFingerprint, "rotationFingerprint");
      if (acceptedKeyIds.size() != 2) {
        throw new IllegalArgumentException(
            "a credential rotation must expose exactly two accepted keys");
      }
    }
  }

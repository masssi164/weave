...
public interface RuntimeWorkloadIdentityInventory {
    ...
    public Snapshot {
      ...
      clients =
          clients == null
              ...
              : clients.stream()
                  .sorted(
                      Comparator.comparing(ClientObservation::clientId)
                          .thenComparing(ClientObservation::providerRef))
                  .toList();
      if (clients.stream().map(ClientObservation::providerRef).distinct().count()
          != clients.size()) {
  ...
  record QuarantineManagedCommand(
      String providerRef, String clientId, String ownerFingerprint, String auditRef) {

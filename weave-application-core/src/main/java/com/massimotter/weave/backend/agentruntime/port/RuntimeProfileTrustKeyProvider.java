...
public interface RuntimeProfileTrustKeyProvider {
    ...
    record TrustKey(String keyId, PublicKey publicKey, Instant validFrom, Instant validUntil) {
        public TrustKey {
            if (keyId == null || keyId.isBlank() || publicKey == null || validFrom == null || validUntil == null
                    || !validUntil.isAfter(validFrom)) {
                throw new IllegalArgumentException("complete RuntimeProfile trust-key metadata is required");
            }

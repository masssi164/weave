...
public final class RunnerControl {
        ...
        public TaskLease {
            ...
            for (URI contextRef : contextRefs) {
                if (!"weave".equals(contextRef.getScheme())
                    ...
                    throw new IllegalArgumentException("contextRef must be an opaque Weave resource URI");
                }
            }
    ...
    private static Map<String, Object> immutableObjectMap(
        ...
        result.forEach((key, value) -> bounded(required(key, field + " key"), 128, field + " key"));
        return result;
    }
}

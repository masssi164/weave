...
public final class RuntimeProfileDeliveryService {
  ...
  public Optional<SignedRuntimeProfile> findCurrent(
      String profileHash, RuntimeWorkloadPrincipal principal) {
    ...
    return profiles
        .findCurrentForWorkload(
            profileHash, principal.issuer(), principal.subject(), principal.clientId(), now)
  ...
  private boolean verifiesFor(
      SignedRuntimeProfile envelope, RuntimeWorkloadPrincipal principal, Instant now) {
    try {

...
fn messages_value(
    ...
    let events = conversation
        ...
        .collect::<Result<Vec<_>, MatrixCoreError>>()?;
    Ok(
        json!({ "start": input.since.clone().unwrap_or_else(|| encode_sync_token("start")), "end": encode_sync_token(&input.cursor), "chunk": events }),
    )
}
...
fn parse_encrypted_content(content: &Value) -> Result<Value, MatrixCoreError> {
    ...
    for (field, max_length) in [("ciphertext", 262_144), ("session_id", 512)] {
        let value = object
            .get(field)
            .and_then(Value::as_str)
            .filter(|v| !v.is_empty() && v.len() <= max_length)
            .ok_or(MatrixCoreError::InvalidRequest)?;
...
fn parse_message_content(
    ...
    let body = object
        ...
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|v| !v.is_empty() && v.len() <= 65_536)
        .ok_or(MatrixCoreError::InvalidRequest)?;
...
fn parse_reaction_content(
    ...
    Ok(json!({
        ...
        "relationKind":"reaction","relationTargetEventId":decode_event_id(event_id,server_name)?,"replyToEventId":Value::Null,"reactionKey":key,
...
fn parse_relation(
    ...
    if let Some(in_reply_to) = object.get("m.in_reply_to").and_then(Value::as_object) {
        ...
        return Ok((
            ...
            Some(decode_event_id(event_id, server_name)?),
            Some(decode_event_id(event_id, server_name)?),
        ));
    }
    let rel_type = object
    ...
    let target = object
        ...
        .transpose()?;
    let kind = match rel_type.as_deref() {
...
fn decode_room_value(
    ...
    let value = id.as_str();
    let separator = value.rfind(':').ok_or(MatrixCoreError::InvalidRequest)?;
...
fn decode_event_value(
    ...
    let value = id.as_str();
    let separator = value.rfind(':').ok_or(MatrixCoreError::InvalidRequest)?;
    Ok(json!({"eventId": &value[1..separator]}))
}
...
fn matrix_error_value(input_json: &str) -> Result<Value, MatrixCoreError> {
    ...
    let errcode = input
        .get("errcode")
        .and_then(Value::as_str)
        .filter(|v| v.starts_with("M_") && v.len() <= 128)
        .ok_or(MatrixCoreError::InvalidRequest)?;
    let error = input
        .get("error")
        .and_then(Value::as_str)
        ...
        .ok_or(MatrixCoreError::InvalidRequest)?;
    Ok(
        json!({"errcode":errcode,"error":error,"weaveBoundary":"northbound-matrix-client-server","canonicalDomain":"chat","supportSafe":true,"providerDataPlaneExposed":false}),
    )
}
    ...
    if let Some(joined) = root.pointer("/rooms/join").and_then(Value::as_object) {
        for (room_id, room) in joined {
            ...
            rooms.push(ClientRoomProjection {
                room_id: room_id.clone(),
                title: room
                    .pointer("/state/events/0/content/name")
                    .and_then(Value::as_str)
                    .unwrap_or("Room")
                    .to_string(),
                unread_count: room
                    .pointer("/unread_notifications/notification_count")
...
fn parse_messages_value(
    ...
    Ok(
        json!({"messages":messages,"end":root.get("end").and_then(Value::as_str).unwrap_or_default()}),
    )
}
...
fn validate_server_name(value: &str) -> Result<OwnedServerName, MatrixCoreError> {
    ...
    OwnedServerName::try_from(value).map_err(|_| MatrixCoreError::InvalidMatrixId {
        kind: "server name",
    })
}
fn matrix_user_id(
...
fn matrix_room_id(id: &str, server_name: &OwnedServerName) -> Result<OwnedRoomId, MatrixCoreError> {
    ...
    OwnedRoomId::try_from(format!("!{}:{}", local, server_name))
        .map_err(|_| MatrixCoreError::InvalidMatrixId { kind: "room id" })
}
fn matrix_event_id(
    id: &str,
    server_name: &OwnedServerName,
) -> Result<OwnedEventId, MatrixCoreError> {
    let local = canonical_localpart(id.trim_start_matches('$'), "event")?;
    OwnedEventId::try_from(format!("${}:{}", local, server_name))
        .map_err(|_| MatrixCoreError::InvalidMatrixId { kind: "event id" })
}
fn validate_user_for_server<'a>(
    value: &'a str,
    server: &OwnedServerName,
) -> Result<&'a str, MatrixCoreError> {
    let id = OwnedUserId::try_from(value)
        .map_err(|_| MatrixCoreError::InvalidMatrixId { kind: "user id" })?;
    if id.server_name().as_str() != server.as_str() {
        return Err(MatrixCoreError::InvalidMatrixId { kind: "user id" });
    }
    Ok(value)
}
fn validate_room_for_server(
    ...
    let id = OwnedRoomId::try_from(value)
        .map_err(|_| MatrixCoreError::InvalidMatrixId { kind: "room id" })?;
    if id.server_name().map(|name| name.as_str()) != Some(server.as_str()) {
        return Err(MatrixCoreError::InvalidMatrixId { kind: "room id" });
    }
    Ok(id)
}
fn validate_event_for_server(
    ...
    let id = OwnedEventId::try_from(value)
        .map_err(|_| MatrixCoreError::InvalidMatrixId { kind: "event id" })?;
    if let Some(name) = id.server_name() {
        if name.as_str() != server.as_str() {
            return Err(MatrixCoreError::InvalidMatrixId { kind: "event id" });
        }
    }
    Ok(id)
}
fn decode_event_id(value: &str, server: &OwnedServerName) -> Result<String, MatrixCoreError> {
    validate_event_for_server(value, server)?;
    let separator = value
        .rfind(':')
        .ok_or(MatrixCoreError::InvalidMatrixId { kind: "event id" })?;
...
fn decode_sync_token(value: &str) -> Result<String, MatrixCoreError> {
    let raw = value
...
fn parse<T: for<'de> Deserialize<'de>>(input: &str) -> Result<T, MatrixCoreError> {
    serde_json::from_str(input).map_err(|_| MatrixCoreError::InvalidRequest)
}
...
fn room_state_events(
    ...
    events.push(MatrixEvent {
        ...
        sender: sender.clone(),
        event_id: matrix_event_id(&format!("{}-name", conversation.conversation_id), server)?
            .to_string(),
        origin_server_ts: conversation.updated_at_epoch_millis,
        content: json!({"name":conversation.title}),
        state_key: Some(String::new()),
    });
    if let Some(algorithm) = &conversation.encryption_algorithm {
        events.push(MatrixEvent {
            event_type: "m.room.encryption".into(),
            sender: sender.clone(),
            event_id: matrix_event_id(
                &format!("{}-encryption", conversation.conversation_id),
                server,
            )?
            .to_string(),
            origin_server_ts: conversation.updated_at_epoch_millis,
...
fn message_event(
    ...
    let content = if message.redacted {
        ...
        json!({"m.relates_to":{"rel_type":"m.annotation","event_id":matrix_event_id(message.relation_target_event_id.as_deref().ok_or(MatrixCoreError::InvalidRequest)?,server)?.to_string(),"key":message.reaction_key.as_deref().ok_or(MatrixCoreError::InvalidRequest)?}})
    } else {
        ...
        object.insert("msgtype".into(), json!(message_type));
        object.insert("body".into(), json!(body));
        if let Some(format) = &message.format {
            object.insert("format".into(), json!(format));
        }
        if let Some(body) = &message.formatted_body {
            object.insert("formatted_body".into(), json!(body));
        }
        for (k, v) in &message.presentation_extensions {
            if matches!(
                k.as_str(),
                "com.openclaw.approval" | "com.openclaw.presentation"
            ) {
        ...
        object.insert("weaveMessageId".into(), json!(message.message_id));
        object.insert("weaveDeliveryState".into(), json!(message.delivery_state));
        object.insert("weaveCanonicalDomain".into(), json!("chat"));
        object.insert("providerDataPlaneExposed".into(), json!(false));
        Value::Object(object)
    };
    Ok(MatrixEvent {
        event_type: if message.kind == "encrypted" {
            "m.room.encrypted"
        } else if message.kind == "reaction" {
            "m.reaction"
        } else {
        ...
        .into(),
        sender,
        event_id: matrix_event_id(&message.message_id, server)?.to_string(),
...
mod tests {
    ...
    fn sync_tokens_are_opaque_and_reversible() {
        let token = encode_sync_token("chat-revision-42|e2ee:5");
        assert_eq!(
            decode_sync_token(&token).unwrap(),
            "chat-revision-42|e2ee:5"
        );
    }
}

...
assert_json "${provider_status}" '[.providers[] | select(.module == "meetings" and .providerKey == "matrix-meetings") | select(.enabled == false and .configured == false and .failClosed == true and .supportSafe == true)] | length == 1' "Meetings/video provider seam should default fail-closed and support-safe"

Feature: Sprint 8 domain control-plane evidence
  ...
  Scenario: Admin validates domains, reviews Identity Ops readiness, checks portability, and members see provider-neutral states
    Given an organization admin opens the Admin Console
    When the admin reviews canonical domain setup
    And reviews support-safe Keycloak Identity Ops plan and verify evidence

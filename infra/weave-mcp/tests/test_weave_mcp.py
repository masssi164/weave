...
class WeaveMcpGatewayTest(unittest.TestCase):
    ...
    def test_openapi_route_map_is_explicit_and_fails_closed_on_drift(self) -> None:
        ...
        drifted["paths"]["/api/admin/control-plane"]["get"]["operationId"] = "renamedAdminReadiness"
        with self.assertRaises(McpDenied) as raised:
            assert_route_map_matches_openapi(drifted)
        self.assertEqual(raised.exception.reason, "openapi-operationid-drift-for-admin.get_readiness")
    ...
    def test_discovery_uses_runtime_profile_projection_not_caller_grant_headers(self) -> None:
        profile = {
            **RUNTIME_PROFILE_PROJECTION,
            "allowedTools": ["admin.get_readiness"],
            "capabilityGrants": ["weaver.admin_readiness_read"],
        }
        ...
        self.assertEqual(set(tools), {"admin.get_readiness"})
    ...
    def test_stale_or_overbroad_runtime_profile_projection_fails_closed(self) -> None:
        ...
        overbroad = {**RUNTIME_PROFILE_PROJECTION, "allowedTools": ["boards.comment", "write calendar"]}
    ...
    def test_local_streamable_http_server_discovery_and_invocation(self) -> None:
        ...
        try:
            ...
            self.assertIn("boards.comment", {tool["name"] for tool in payload["tools"]})

            invoke = Request(
                base + "/mcp/invoke",
                method="POST",
                headers={**HEADERS, "Content-Type": "application/json"},
                data=json.dumps({"tool": "admin.get_readiness", "input": {}}).encode(),
            )
            with urlopen(invoke, timeout=5) as response:
                result = json.loads(response.read())
            self.assertTrue(result["supportSafe"])
            self.assertTrue(result["result"]["supportSafe"])

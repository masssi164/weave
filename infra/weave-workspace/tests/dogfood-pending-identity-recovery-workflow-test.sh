...
require_workflow 'credential_state="${XDG_STATE_HOME:-$HOME/.local/state}/weave/dogfood/bootstrap.env"'
require_workflow 'Private persistent bootstrap credential state is unavailable.'
require_workflow 'mv "$bootstrap_staging" "$generated_root/bootstrap.env"'
require_workflow 'Persistent ARC trust state is partial; refusing to overwrite it.'
require_workflow 'docker inspect weave-backend --format'
require_workflow '/run/weave-agent-runtime/profile-signing'
require_workflow 'Running backend ARC mounts do not form one complete trust authority.'
require_workflow 'mv "$arc_staging" "$arc_target"'
...
assert_workflow_order 'mv "$bootstrap_staging" "$generated_root/bootstrap.env"' 'bash ./backup.sh "$backup_root"'
assert_workflow_order 'mv "$arc_staging" "$arc_target"' 'bash ./backup.sh "$backup_root"'

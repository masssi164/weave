...
assert_file_contains "${install_script}" 'normalize every persisted value to an exported variable'
assert_file_contains "${install_script}" 'export "${var}=${!var}"'
...
assert_file_contains_twice "${keycloak_main}" 'client_secret_regenerate_when_changed = { rotation_epoch = var.admin_client_secret_rotation_epoch }'
assert_file_contains "${install_script}" 'reconcile_keycloak_generated_admin_credentials'
assert_file_contains "${install_script}" 'reconcile_unimportable_keycloak_client_policy_state'
assert_file_contains "${install_script}" 'module.tenant_identity.keycloak_realm_client_policy_profile_policy.token_exchange_downscope'
assert_file_contains "${install_script}" 'module.tenant_identity.keycloak_realm_client_policy_profile.token_exchange_downscope'
assert_file_contains "${install_script}" '/client-policies/policies'
assert_file_contains "${install_script}" '/client-policies/profiles'
assert_file_contains "${install_script}" 'refusing ambiguous reconciliation'
assert_file_contains "${install_script}" 'keycloak_admin_put'
...
assert_file_contains "${keycloak_main}" 'client_scope_id = keycloak_openid_client_scope.weaver_runtime_workload.id'
assert_file_contains "${keycloak_main}" 'role_id         = keycloak_role.weaver_runtime.id'
assert_file_contains "${keycloak_main}" 'name                   = "agent-runtime.admin"'
assert_file_contains "${keycloak_main}" 'name                   = "mcp:tools"'
assert_file_contains "${keycloak_main}" 'name                   = "weave-mcp-backend.exchange"'
assert_file_contains "${keycloak_main}" 'client_scope_id          = keycloak_openid_client_scope.mcp_backend_exchange.id'
assert_file_contains "${keycloak_main}" 'default_scopes = [keycloak_openid_client_scope.mcp_backend_exchange.name]'
assert_file_contains "${keycloak_main}" 'included_client_audience = keycloak_openid_client.client["weave_backend"].client_id'
assert_file_contains "${keycloak_main}" 'optional_scopes = [keycloak_openid_client_scope.agent_runtime_admin.name]'
assert_file_contains "${keycloak_main}" 'included_custom_audience = "${var.api_public_url}/api/v1/agent-runtime"'
assert_file_contains "${keycloak_main}" 'included_custom_audience = "${var.api_public_url}/mcp"'
assert_file_contains "${keycloak_main}" 'name        = "weaver-runtime"'
assert_file_contains "${backend_main}" 'WEAVE_AGENT_RUNTIME_ADMIN_CLIENT_ID=${var.agent_runtime_admin_client_id}'
assert_file_contains "${backend_main}" 'WEAVE_AGENT_RUNTIME_ENTITLEMENT_CLIENT_ID=${var.agent_runtime_entitlement_client_id}'
assert_file_contains "${backend_main}" 'WEAVE_AGENT_RUNTIME_ENTITLEMENT_CREDENTIAL_REF=${var.agent_runtime_entitlement_credential_ref}'
assert_file_contains "${backend_main}" 'WEAVE_AGENT_RUNTIME_DEFAULT_CLIENT_SCOPES=weaver-runtime.workload'
assert_file_contains "${install_script}" 'module.tenant_identity.keycloak_generic_role_mapper.weaver_runtime_workload'
assert_file_contains "${install_script}" 'module.tenant_identity.keycloak_openid_client_scope.mcp_backend_exchange:weave-mcp-backend.exchange'
assert_file_contains "${install_script}" 'keycloak_lookup_client_scope_realm_role_id'
assert_file_absent "${keycloak_main}" 'name                   = "weave:mcp"'
assert_file_absent "${keycloak_main}" 'name                   = "weave:mcp-backend"'
assert_file_contains "${mcp_main}" 'WEAVE_OIDC_ISSUER_URI=${var.oidc_issuer_uri}'
assert_file_absent "${mcp_main}" 'WEAVE_MCP_CLIENT_SECRET='
assert_file_absent "${mcp_main}" 'WEAVE_MCP_INBOUND_AUTHORIZED_PARTY='
...
assert_file_contains "${REPO_DIR}/README.md" 'Matrix v1.19 plus pinned MatrixRTC Profile 0 is the Calls target; the obsolete member Calls API and proprietary LiveKit join-grant slice are removed.'
...
assert_file_contains "${keycloak_main}" '"manage-users"'
assert_file_contains "${keycloak_main}" '"manage-clients"'

...
main() {
  ...
  jq -e '.schemaVersion == "weave.compose-private-backup-integrity.v3" and (.profile == "dogfood" or .profile == "test")' \
    "${TEMP_ROOT}/integrity.json" >/dev/null || fail "backup is not a verified dogfood Compose v3 consistency set"

...
grep -Fq "http://${TF_VAR_tenant_domain:-weave.local}:${TF_VAR_proxy_http_host_port:-44080}/weave-local-ca.pem" <<<"${product_start_page}" || fail "Smoke check failed: start page should include the HTTP local CA download link"

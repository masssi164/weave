...
mcp_resource="$(public_url "${TF_VAR_api_subdomain:-api}")/mcp"
mcp_metadata="$(curl_json "$(public_url "${TF_VAR_api_subdomain:-api}")/.well-known/oauth-protected-resource/mcp")"
...
grep -Fq '<h1>Weave Local Dogfood start</h1>' <<<"${product_start_page}" || fail "Smoke check failed: product gateway should render the local dogfood start page"
grep -Fq "http://${TF_VAR_tenant_domain:-weave.test}:${TF_VAR_proxy_http_host_port:-44080}/weave-local-ca.pem" <<<"${product_start_page}" || fail "Smoke check failed: start page should include the HTTP local CA download link"
grep -Fq "${WEAVE_PUBLIC_BASE_URL}/weave-local-ca.pem" <<<"${product_start_page}" || fail "Smoke check failed: start page should include the HTTPS local CA download link"
...
grep -Fq 'passwords, tokens, client secrets, credential URLs, or activation action links' <<<"${product_start_page}" || fail "Smoke check failed: start page should warn that secrets are not embedded"
grep -Fq 'Account activation uses the identity-provider required-action flow' <<<"${product_start_page}" || fail "Smoke check failed: start page should describe the required-action activation boundary"
...
invite_link="$(${ROOT_DIR}/local-invite-link.sh --handoff-ref "${invite_handoff_ref}" --run-id s32-invite --base-url "${WEAVE_PUBLIC_BASE_URL}")"
...
default_invite_json="$(${ROOT_DIR}/local-invite-link.sh --json --base-url "${WEAVE_PUBLIC_BASE_URL}")"
jq -e --arg platform_config_url "${WEAVE_PUBLIC_BASE_URL}/api/platform/config" \
  '.inviteLink == .qrPayload and .org == "massimo-dogfood" and .workspace == "home" and .platformConfigUrl == $platform_config_url' \
  >/dev/null <<<"${default_invite_json}" || fail "Smoke check failed: Default invite generator should emit compact no-secret JSON for Massimo home"

log "Checking QR payload onboarding contract..."
qr_handoff_ref="handoff-s32-qr"
qr_payload="$(${ROOT_DIR}/local-invite-link.sh --handoff-ref "${qr_handoff_ref}" --run-id s32-qr --base-url "${WEAVE_PUBLIC_BASE_URL}")"
...
assert_json "${platform_config}" ".schemaVersion == 1 and .organizationOrigin == \"${WEAVE_PUBLIC_BASE_URL}\" and .controlPlaneBaseUrl == \"${WEAVE_BASE_URL}\"" "Platform config should expose canonical Weave organization and control-plane routes"
assert_json "${platform_config}" ".oidc.issuer == \"${WEAVE_OIDC_ISSUER_URL}\" and .oidc.clientId != \"\"" "Platform config should expose the mandatory OIDC contract"
assert_json "${platform_config}" ".protocols.matrixClientServerBaseUrl == \"${WEAVE_MATRIX_HOMESERVER_URL}\"" "Platform config should expose the Weave Matrix Client-Server facade"
assert_json "${platform_config}" ".protocols.filesWebDavBaseUrl == \"${WEAVE_BASE_URL}/dav/files\" and .protocols.calendarCalDavBaseUrl == \"${WEAVE_BASE_URL}/caldav\"" "Platform config should expose separate Weave WebDAV and CalDAV facades"
assert_json "${platform_config}" '([.domains[].domain] | sort) == (["boards","calendar","chat","files","health","identity"] | sort)' "Platform config should include every active dogfood domain"
assert_json "${platform_config}" '((has("publicBaseUrl") | not) and (has("apiBaseUrl") | not) and (has("matrixHomeserverUrl") | not) and (has("nextcloudBaseUrl") | not) and (has("targets") | not) and (has("features") | not))' "Platform config must not expose provider-shaped legacy fields"

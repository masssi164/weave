...
- `COMPOSE_PROFILES` in the reviewed environment file selects exactly one environment profile.
  Optional `provider-matrix`, `provider-nextcloud`, `storage-s3`, and `dev-tools` profiles are
  enabled only when their matching public provider/storage inputs require them.

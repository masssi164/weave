...
OpenProject is the first real provider-backed Boards path for Weave, but Weave remains the product UX. OpenProject is a backend/provider engine only; clients use the Weave Boards workspace facade and never receive provider tokens or raw provider URLs.

## Default posture

- The core local stack does not require OpenProject.
- `TF_VAR_boards_runtime_enabled=false` and `TF_VAR_boards_provider=local-workspace` by default.
- `TF_VAR_boards_openproject_runtime_enabled=false` and `TF_VAR_boards_openproject_read_sync_enabled=false` by default.
- `TF_VAR_boards_openproject_provider_writes_enabled=false` for the v0.1 provider path.
...
TF_VAR_boards_provider=openproject
...
TF_VAR_boards_openproject_api_token=replace-with-service-token
```
...
Then configure the backend provider gate to use the Docker-network URL:
...
By default this validates the safe default posture: `/api/boards/workspace` is reachable only through Weave and fails closed with a support-safe Boards error, and provider writes are refused without leaking OpenProject secrets, raw provider URLs, or `/api/v3` paths. The manual full-stack smoke workflow runs this fail-closed gate after the core smoke test.

For an enabled provider-backed run, first configure the backend with the OpenProject variables above, point it at either the external connector or the optional self-hosted profile, and ensure Context/Space authorization has a membership for the smoke-test principal. Then run:
...
The enabled mode requires the response source to be `openproject-workspace-sync-backend-facade`, provider capabilities to report `openproject`, sync metadata to be user-write-audited/context-scoped/support-safe, and provider-neutral projects, boards, and tasks to be present. It also rechecks that provider write attempts remain refused until the later audit/consent provider-write promotion.
...
5. Live Stack E2E proves provider-backed Boards alongside Chat, Calendar, and Files.

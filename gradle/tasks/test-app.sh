...
FAILURE_DIAGNOSTICS_ROOT="${WEAVE_TEST_APP_FAILURE_DIAGNOSTICS_ROOT:-}"
...
if [[ -z "${FAILURE_DIAGNOSTICS_ROOT}" ]]; then
  FAILURE_DIAGNOSTICS_ROOT="${WEAVE_TEST_APP_RUN_ROOT}/failure-diagnostics"
fi
[[ "${FAILURE_DIAGNOSTICS_ROOT}" == /* ]] ||
  fail "WEAVE_TEST_APP_FAILURE_DIAGNOSTICS_ROOT must be absolute"
if [[ -e "${FAILURE_DIAGNOSTICS_ROOT}" ]]; then
  [[ -d "${FAILURE_DIAGNOSTICS_ROOT}" && ! -L "${FAILURE_DIAGNOSTICS_ROOT}" ]] ||
    fail "WEAVE_TEST_APP_FAILURE_DIAGNOSTICS_ROOT is unsafe"
fi

...
final class McpBearerChallengeWriter {
  ...
  void insufficientScope(HttpServletResponse response) throws IOException {
    challenge(
        ...
        403,
        "insufficient_scope",
        String.join(" ", properties.requiredScopes()),
  ...
  void badRequest(HttpServletResponse response) throws IOException {
    response(
        ...
        400,
        "invalid_request",
        "The MCP request does not satisfy the negotiated extension contract.");
  }
  ...
  void unavailable(HttpServletResponse response) throws IOException {
    response(
        ...
        "The MCP authorization authority is unavailable.");
  }

  private void challenge(
      HttpServletResponse response, int status, String error, String scope, String description)
      throws IOException {
    StringBuilder value =
        new StringBuilder("Bearer resource_metadata=\"")

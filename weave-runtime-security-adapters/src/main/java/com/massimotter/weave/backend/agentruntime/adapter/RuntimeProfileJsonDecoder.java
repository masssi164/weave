...
final class RuntimeProfileJsonDecoder {
  ...
  private static RuntimeProfile.WorkloadIdentity workload(JsonNode node) {
    ...
    return new RuntimeProfile.WorkloadIdentity(
        ...
        text(node, "role"),
        enumeration(
            RuntimeProfile.AuthenticationMethod.values(),
            text(node, "authenticationMethod"),
            RuntimeProfile.AuthenticationMethod::wireValue,
  ...
  private static RuntimeProfile.ModelPolicy modelPolicy(JsonNode node) {
    fields(
        node,
        Set.of("allowedProviders", "allowedModels", "fallback"),
        Set.of("maximumContextTokens", "dataRegion"));
    Integer maximum =
        node.has("maximumContextTokens") ? integer(node, "maximumContextTokens") : null;
    return new RuntimeProfile.ModelPolicy(
        strings(node, "allowedProviders"),
        strings(node, "allowedModels"),
        strings(node, "fallback"),
        maximum,
        optionalText(node, "dataRegion"));
  }

  private static RuntimeProfile.MatrixPolicy matrixPolicy(JsonNode node) {
    fields(
        node,
        Set.of("accountRef", "homeserverRef", "allowedRooms", "encryptionRequired"),
        Set.of("credentialRef", "autoJoin"));
    RuntimeProfile.AutoJoin autoJoin =
        node.has("autoJoin")
            ? enumeration(
                RuntimeProfile.AutoJoin.values(),
                text(node, "autoJoin"),
                RuntimeProfile.AutoJoin::wireValue,
                "autoJoin")
            : null;
    return new RuntimeProfile.MatrixPolicy(
        text(node, "accountRef"),
        text(node, "homeserverRef"),
        optionalText(node, "credentialRef"),
        strings(node, "allowedRooms"),
        autoJoin,
        bool(node, "encryptionRequired"));
  }
  ...
  private static RuntimeProfile.McpServer mcpServer(JsonNode node) {
    ...
    return new RuntimeProfile.McpServer(
        text(node, "serverRef"),
        text(node, "endpoint"),
        text(node, "requestedResource"),
        text(node, "extensionId"),
        text(node, "grantType"),
        strings(node, "requiredScopes"),
  ...
  private static RuntimeProfile.ApprovalPolicy approvalPolicy(JsonNode node) {
    ...
    return new RuntimeProfile.ApprovalPolicy(
        text(node, "owner"),
        pluginRouting(object(node, "pluginRouting")),
        enumeration(
            RuntimeProfile.ExecMode.values(),
            text(node, "execMode"),
            RuntimeProfile.ExecMode::wireValue,
            "execMode"),
        enumeration(
            RuntimeProfile.PersistentTrustPolicy.values(),
            text(node, "persistentTrustPolicy"),
            RuntimeProfile.PersistentTrustPolicy::wireValue,
  ...
  private static RuntimeProfile.PluginRouting pluginRouting(JsonNode node) {
    ...
    return new RuntimeProfile.PluginRouting(
        bool(node, "enabled"),
        enumeration(
            RuntimeProfile.PluginRoutingMode.values(),
            text(node, "mode"),
            RuntimeProfile.PluginRoutingMode::wireValue,
            "plugin routing mode"),
  ...
  private static RuntimeProfile.SandboxPolicy sandboxPolicy(JsonNode node) {
    fields(
        node,
        Set.of("mode", "networkPolicy", "filesystemPolicy"),
        Set.of("allowedNetworkTargets", "approvedMountRefs"));
    return new RuntimeProfile.SandboxPolicy(
        enumeration(
            RuntimeProfile.SandboxMode.values(),
            text(node, "mode"),
            RuntimeProfile.SandboxMode::wireValue,
            "sandbox mode"),
        enumeration(
            RuntimeProfile.NetworkPolicy.values(),
            text(node, "networkPolicy"),
            RuntimeProfile.NetworkPolicy::wireValue,
            "networkPolicy"),
        node.has("allowedNetworkTargets") ? strings(node, "allowedNetworkTargets") : null,
        enumeration(
            RuntimeProfile.FilesystemPolicy.values(),
            text(node, "filesystemPolicy"),
            RuntimeProfile.FilesystemPolicy::wireValue,
            "filesystemPolicy"),
  ...
  private static RuntimeProfile.AutomationPolicy automationPolicy(JsonNode node) {
    ...
    return new RuntimeProfile.AutomationPolicy(
        ...
        enumeration(
            RuntimeProfile.SchedulePolicy.values(),
            text(node, "schedulePolicy"),
            RuntimeProfile.SchedulePolicy::wireValue,
            "schedulePolicy"));
  }

...
public final class Ed25519JcsRuntimeProfileVerifier implements RuntimeProfileVerifier {
  ...
  public Ed25519JcsRuntimeProfileVerifier(
      ObjectMapper objectMapper, RuntimeProfileTrustKeyProvider trustKeys) {
  ...
  public RuntimeProfile verify(SignedRuntimeProfile envelope, Instant now) {
    ...
    try {
      ...
      RuntimeProfileTrustKeyProvider.TrustKey trustKey =
          trustKeys
              .resolve(envelope.keyId(), now)
      ...
      String hash =
          "sha256:"
              + HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(payloadBytes));
      ...
      verifier.update(
          (envelope.protectedHeader() + "." + envelope.payload())
  ...
  private static void requireHeader(JsonNode header, String expectedKeyId) {
    ...
    header.properties().forEach(entry -> actual.add(entry.getKey()));

...
- [ ] T011 [Acceptance] Create `e2e/features/weave_spec_0011_acceptance.feature` for eligibility, policy preview, memory isolation, approvals, audit, heartbeat/fallback.
- [ ] T012 [Acceptance] Map Weaver target scenarios in `e2e/scenario_mappings.json`.
...
- [ ] T020 [US1/Admin] Add/repair Admin Console Weaver policy + `weaver-group` eligibility preview; paths `admin-console/`, `server/`, `e2e/`; gate `./gradlew adminCi serverCi acceptanceContract`.
- [ ] T021 [US2/Runtime] Add/repair signed per-user `WeaverRuntimeProfile` generation and revocation; paths `server/`; gate `./gradlew serverCi`.
- [ ] T022 [US2/Memory] Add/repair per-user memory scope/export/delete/retention contract; paths `server/`, `e2e/`; gate `./gradlew serverCi acceptanceContract`.
- [ ] T023 [US2/Tools] Add/repair domain-first MCP tool discovery/execution with scopes and provider facade calls; paths `server/`, `e2e/`; gate `./gradlew serverCi acceptanceContract`.
- [ ] T024 [US2/Approvals] Add/repair approval receipt validation, stale-version fail-closed behavior, and accessible UX hooks; paths `server/`, `client/`, `e2e/`; gate `./gradlew serverCi clientCi acceptanceContract`.
- [ ] T025 [US3/Audit] Add/repair support-safe audit for model/channel/memory/tool/approval/denial/heartbeat events; paths `server/`, `admin-console/`, `e2e/`; gate `./gradlew serverCi adminCi acceptanceContract`.
- [ ] T026 [US2/Automation] Add/repair heartbeat/automation schedule/scope/rate-limit policy and denial evidence; paths `server/`, `admin-console/`; gate `./gradlew serverCi adminCi`.

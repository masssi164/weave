...
- **FR-014**: Chat provider changes MUST be modeled as admin-governed Weave Chat domain provider migrations. The profile generator MUST expose OpenClaw's stock `channels.matrix` plugin against the OIDC-gated Weave northbound Matrix Client-Server facade. The selected southbound provider, provider credentials, and provider identifiers MUST remain behind Weave Chat-domain routing; members MUST NOT switch raw southbound adapters or channel tokens.
...
| Chat channel projection | OpenClaw stock `channels.matrix` configuration with the Weave Matrix-facade URL, facade Matrix identifiers, a short-lived access-token `SecretRef`, native approval routing, and runtime-profile correlation. ProviderRefs such as `matrix`, `teams`, `imessage`, or `slack` stay behind Weave Chat-domain routing and are not rendered as alternative per-user channel configs. |
...
Provider-change flow: Admin changes the Weave Chat domain provider -> server readiness and migration checks run -> Credential Broker binds new provider credentials -> Weave backend Chat routing/profile version changes -> `WeaverRuntimeProfile` vNext is generated and signed with the same northbound `channels.matrix` facade contract and updated metadata -> channel/runtime reloads or restarts if needed -> member continues through Weave UX.

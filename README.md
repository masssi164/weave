...
| Weaver | Guarded workload-only OAuth/MCP boundary at `/mcp` using Spring AI stateful Streamable HTTP | Per-cell RuntimeProfile v2 tools, signed single-use ApprovalDecisionEvidence v2, current domain authorization, and immutable ActionEvidence v2; human clients have no MCP access | Edge remains dark until ARC proves per-cell Keycloak workload identity and lifecycle reconciliation |
...
- The Weaver MCP process is installed but intentionally dark: Spring Security rejects human tokens and every unbound workload before protocol dispatch. The obsolete member-oriented v1 catalog, token exchange, runtime routes, fake Scout UI, and approval flow have been removed. ARC must prove per-cell Keycloak client lifecycle, RuntimeProfile v2 binding, signed single-use ApprovalDecisionEvidence v2, current authorization, and immutable ActionEvidence v2 before the edge can open.

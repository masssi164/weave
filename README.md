...
| Weaver | OIDC-protected MCP at `/mcp` using Spring AI stateful Streamable HTTP | RuntimeProfile-filtered domain tools, resources, prompts, signed single-use approval decision evidence, current domain authorization, and immutable action evidence | Weave canonical use cases; downstream providers are never MCP authorities |

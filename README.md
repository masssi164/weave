...
Weaver, the AI assistance layer, follows that product order: optional, policy-bound, auditable, and disabled until an organization chooses to enable it. Its locked target is a disposable OpenClaw cell with zero durable bytes; current runtime behavior remains guarded until external checkpoint, workspace, secret, lease/fencing, and cross-node restore evidence exists.
...
| Boards / Tasks | Weave product/control APIs | Provider-neutral task and board contracts, readiness, approvals, and audit | OpenProject-class adapters as configured |
| Calls & Meetings | Matrix v1.19 Client-Server plus guarded `weave.matrixrtc/profile-0` | Matrix slot/member state, RTC authorization policy, consent, governed artifacts; no member Calls REST data plane | LiveKit first as a replaceable MatrixRTC transport/SFU; implementation pending |
| Weaver / Agent Runtime Control | Matrix interaction plus OIDC-protected MCP; signed runtime-control contracts | Optional governed personal assistant; the target cell owns zero durable bytes | OpenClaw/Weaver target with WebDAV, Control Store, encrypted RuntimeStateStore, and Secret Manager/KMS as separate authorities |

Spring Boot is the server gatekeeper for authorization, audit, readiness, and support-safe errors. The Matrix facade shares the public API origin; a `matrix.<tenant>` host is a southbound provider/operator endpoint, never a member-client setting. For Matrix clients, Matrix Authentication Service is the Native OAuth authorization server and uses Keycloak as its upstream organizational identity provider. Calls use Matrix/MatrixRTC as the member contract; LiveKit is transport, not a Weave Calls API. Matrix protocol shaping targets a shared Rust/Ruma core: server integration through JNI, Flutter integration through `flutter_rust_bridge`. Flutter consumes Weave-owned facades, not raw provider SDKs or provider secrets.

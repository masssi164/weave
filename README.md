...
| Weaver | Workload-only OAuth-protected MCP at `/mcp` using Spring AI stateful Streamable HTTP | Per-cell RuntimeProfile-filtered domain tools, signed single-use approval decision evidence, current domain authorization, and immutable action evidence; human clients have no MCP access | Weave canonical use cases; downstream providers are never MCP authorities |
...
- The governed Weaver projection uses Spring AI 2.0 at `/mcp`. Its currently exposed read tools call the same canonical application services as the protocol facades, with RuntimeProfile filtering and support-safe results. Write-like tools remain undiscoverable and fail closed until trusted OpenClaw decision evidence, current domain authorization, single-use consumption, and immutable `ActionEvidence` are proven end to end.

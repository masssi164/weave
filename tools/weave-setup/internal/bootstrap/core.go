...
func BuildPlan(req Request, now time.Time) (Result, error) {
	...
	return Result{Plan: plan, Files: []GeneratedFile{{Path: ".weave/setup/bootstrap-plan.json", Content: planJSON}, {Path: fmt.Sprintf(".weave/setup/workflows/%s-setup.plan.yml", target.Target), Content: workflow}}}, nil
}
...
func renderWorkflowPlan(plan Plan) string {
	secretNames, _ := json.Marshal(plan.Target.RequiredSecretNames)
	return fmt.Sprintf(`# Generated by weave-setup for issue #666. Support-safe plan only; no secret values.
version: 1
selectedTarget: %s
remoteName: %s
branch: %s
githubSecretsRequired: %t
requiredSecretNames: %s
nextActions:
  - validate selected target runner readiness
  - dispatch only through the selected target after explicit approval
`, plan.Target.Target, plan.Target.RemoteName, plan.Target.Branch, plan.Safety.GitHubSecretsRequired, string(secretNames))
}

...
void main() {
  group('NextcloudAuthClient', () {
    test('connect completes the Login Flow v2 handshake and resolves the user id', () async {
    ...
    test('connect rejects credentials returned for a different server', () async {
      final client = MockClient((request) async {
        if (request.url.path.endsWith('/index.php/login/v2')) {
          return http.Response(
            jsonEncode({

...
class HttpWeaveApiClient implements WeaveApiClient {
  ...
  Future<ProviderRegistryStatus> fetchProviderStatus({
    ...
    return ProviderRegistryResponseDto.fromJson(payload).toEntity();
  }
  ...
  Future<OfficeLaunchSession> launchOfficeSession({
    ...
    final payload = await _postJson(
      ...
      failureMessage: 'The Office provider is not available right now.',
      invalidPayloadMessage:
          'The Weave backend returned an invalid Office launch payload.',
      decodeFailureMessage:
          'Unable to decode Office launch data from the Weave backend.',
    );

    return OfficeLaunchResponseDto.fromJson(payload).toEntity();
  }
